...
3. 记录此时夹爪的位姿 位置 $p_{\text{base}}^{\text{tool}}=(x,y,z)$ $p^{\text{tool orgin}}_{\text{base}} = (x,y,z)$ 和姿态（固定角） $\alpha_{base}^{tool}=(\alpha_x,\alpha_y,\alpha_z)$，计算出 ${^{\text{base}}_{\text{tool}}R}$ 和 $^{\text{base}}_{\text{tool}}t$ 
